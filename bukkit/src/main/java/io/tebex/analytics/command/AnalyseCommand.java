...
public class AnalyseCommand implements TabExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        if(args.length == 0) {
            sender.sendMessage("§8[Analytics] §7Welcome to Analytics!");
            sender.sendMessage("§8[Analytics] §7This server is running version §fv" + commandManager.getPlatform().getDescription().getVersion() + "§7.");
            return true;
        }

        Map<String, SubCommand> commands = commandManager.getCommands();
        if(! commands.containsKey(args[0].toLowerCase())) {
            sender.sendMessage("§8[Analytics] §7Unknown command.");
            return true;
        }

        final SubCommand subCommand = commands.get(args[0].toLowerCase());
        if (! sender.hasPermission(subCommand.getPermission())) {
            sender.sendMessage("§b[Analytics] §7You do not have access to that command.");
            return true;
        }
